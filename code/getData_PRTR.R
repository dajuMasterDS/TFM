...
#Data available from 2007 to 2016
for(i in 2007:2016){
  download.file(url=paste("http://www.prtr-es.es/informes/descargas/PRTR_Espana_MAPAMA_residuos", i, ".zip", sep=""), 
                destfile=paste("data/zip/PRTR_Espana_MAPAMA_residuos", i, ".zip", sep="") ,method = "auto")
  unzip(paste("./data/zip/PRTR_Espana_MAPAMA_residuos", i ,".zip", sep=""), files = NULL, list = FALSE, overwrite = TRUE,
        junkpaths = FALSE, exdir = "./data/Waste", unzip = "internal",
        setTimes = FALSE)
}

...
##------------------------------------------------------------------##
## selección año 2015 y provincias de Andalucía
##------------------------------------------------------------------##
...
for (file in files){
  ...
  data = data[data$ProvinciaReside %in% c(4,11,14,18,21,23,29,41),]
...
DfDeaEmiReduc <- subset( DeathsEmissions, select = -c(X,ProvinciaReside,MunicipioReside, 
                                                        ...
                                                        LatitudE,LongitudE,CantidadTotalkg))
...
#nueva variable para causa de muerte por cancer
...
set.seed(1234) 
SAMPLE = sample.split(DfDeaEmiReduc$CausaMuertebasC, SplitRatio = .70)
